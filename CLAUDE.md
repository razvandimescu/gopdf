...
- `recover.go` / `assemble.go` — `Document.computeRecovery`, internal and undelivered (only `Page.OutlineHint` is public): clusters glyph candidates into `glyphShape`s, has a caller-supplied labeller name them, then places the labelled glyphs on baselines and splits words, returning spans per page and a report that accounts for every glyph; the `Document` is not changed. Experimental: every constant in `assemble.go` was measured on one producer, and the acceptance gates are `TestRecoverOutlinesKeepsLinesThatBelong` and the second list in `TestLookAlikesByCase`. The private evaluations — `recover_rfq_test.go` (E1/E2) and the private corpus's `e3/` (E3–E5, joined through `go test -overlay`) — call `computeRecovery` and read `assembly.runs`, so keep both
