...
func TestSearchText(t *testing.T) {
	...
	results := doc.Search("Nova Facilities")
	if len(results) == 0 {
		t.Fatal("expected to find 'Nova Facilities'")
	}
	r := results[0]
	if r.Page != 0 {
...
func TestSearchMultipleResults(t *testing.T) {
	...
	data, _ := os.ReadFile(testDir + "Riverside Shower Block Park Lane Leeds LS1 2AB.pdf")
	doc, err := OpenBytes(data)
	if err != nil {
		t.Fatal(err)
	}

	results := doc.Search("SC-0047/WH")
	if len(results) < 2 {
		t.Errorf("expected at least 2 results for 'SC-0047/WH', got %d", len(results))
	}
...
func TestTextOverlay(t *testing.T) {
	...
	if !strings.Contains(text, "Nova Facilities") {
		t.Error("original text 'Nova Facilities' missing from output")
	}
}
...
func TestRedactText(t *testing.T) {
	...
	ed := NewEditor(data)
	err := ed.RedactText("QT10001", 0, 0, 0) // black redaction
	if err != nil {
...
func TestOverlayAndRedactCombined(t *testing.T) {
	...
	ed.RedactText("QT10001", 1, 1, 1) // white redaction
