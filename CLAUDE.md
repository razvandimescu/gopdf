...
- `recover.go` / `assemble.go` — `Document.RecoverOutlines`: clusters the document's glyph candidates into `GlyphShape`s, has a caller-supplied labeller name them, then places labelled glyphs on baselines and splits words (`assemble.go`, the design's §5). Experimental, and every constant in `assemble.go` was measured on one producer. The result is computed without side effects (`recoverOutlines`) and installed only on success into `Document.recovered`, which `Page.TextSpans` appends. The private-tier evaluation (`recover_rfq_test.go`) reproduces the design's E1/E2 numbers from `outlined_rfq.{pdf,labels.json,reference.txt}` in the private corpus
